...
[![dsh plugin health](https://ice5kysl.github.io/dsh-plugin-insights/badge/ice5kysl/dsh-file-explorer-kit.svg)](https://ice5kysl.github.io/dsh-plugin-insights/)
...
```bash
npm pack          # 产出 dsh-file-explorer-kit-0.3.1.tgz（含预构建 lib/，prepack 自动 build）
# 其他机器：dsh plugin --profile web add ./dsh-file-explorer-kit-0.3.1.tgz
```

> npm 包名：**dsh-file-explorer-kit** —— npm 上的 `dsh-file-explorer` 属于另一个无关插件（joejojoking-cloud），安装请一律用 `dsh plugin add dsh-file-explorer-kit`，不要 `dsh plugin add dsh-file-explorer`。
...
cordis.patch.yml   bundle 层：插入唯一 Loader 入口 dsh-file-explorer-kit
```
